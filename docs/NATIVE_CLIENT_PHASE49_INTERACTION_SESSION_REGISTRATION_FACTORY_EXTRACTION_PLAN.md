# Native Client Phase 49 Interaction Session Registration Factory Extraction Plan

Last updated: 2026-04-06

## Goal
Continue thinning `InteractionSessionHostFactory` by extracting registration host assembly into a focused `InteractionSessionRegistrationFactory` component.

## Execution Slices
1. `49.1` Define Phase 49 scope, artifacts, and completion gates.
2. `49.2` Extract registration host assembly from `InteractionSessionHostFactory` into focused factory ownership.
3. `49.3` Run Phase 49 verification + guard pack and record `PHASE 49 COMPLETE`.

## Phase 49 Slice Status
- `49.1` complete.
- `49.2` complete.
- `49.3` complete.

## Phase 49.1 Outputs
- Added dedicated Phase 49 plan and completion gate references:
  - `docs/NATIVE_CLIENT_PHASE49_INTERACTION_SESSION_REGISTRATION_FACTORY_EXTRACTION_PLAN.md`
- Updated migration/task/status artifacts with Phase 49 lifecycle markers:
  - `docs/NATIVE_CLIENT_MIGRATION_PLAN.md`
  - `docs/NATIVE_CLIENT_PHASE_STATUS.md`
  - `TASKS.md`

## Phase 49.2 Outputs
- Added focused registration factory component:
  - `runelite-plugin/src/main/java/com/xptool/sessions/InteractionSessionRegistrationFactory.java`
- Updated `InteractionSessionHostFactory` registration host assembly methods to route through focused factory ownership:
...
## Phase 49.3 Outputs
- Added explicit Phase 49 verification script:
  - `scripts/verify_phase49_interaction_session_registration_factory_extraction.py`
- Executed Phase 49 verification + guard pack:
...
## Exit Criteria
- `InteractionSessionHostFactory` registration host assembly routes through focused `InteractionSessionRegistrationFactory` ownership.
- `InteractionSessionRegistrationFactory` provides registration host creation boundaries with focused regression tests.
- Phase 49 verification script and native guard pack both pass.
- `PHASE 49 COMPLETE` is recorded in `docs/NATIVE_CLIENT_PHASE_STATUS.md`.
