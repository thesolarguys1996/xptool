from __future__ import annotations

from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PHASE204_PLAN = (
    PROJECT_ROOT
    / "docs/NATIVE_CLIENT_PHASE204_INTERACTION_SESSION_FACTORY_ASSEMBLY_RUNTIME_ASSEMBLY_TYPED_ROUTING_EXTRACTION_PLAN.md"
)
MIGRATION_PLAN = PROJECT_ROOT / "docs/NATIVE_CLIENT_MIGRATION_PLAN.md"
PHASE_STATUS = PROJECT_ROOT / "docs/NATIVE_CLIENT_PHASE_STATUS.md"
TASKS = PROJECT_ROOT / "TASKS.md"
ASSEMBLY_RUNTIME_SESSION_FACTORY = (
    PROJECT_ROOT
    / "runelite-plugin/src/main/java/com/xptool/sessions/InteractionSessionFactoryAssemblyRuntimeSessionFactory.java"
)
ASSEMBLY_RUNTIME_ASSEMBLY_SESSION_FACTORY = (
    PROJECT_ROOT
    / "runelite-plugin/src/main/java/com/xptool/sessions/InteractionSessionFactoryAssemblyRuntimeAssemblySessionFactory.java"
)


def _read(path: Path) -> str:
    return path.read_text(encoding="utf-8")


def main() -> int:
    errors: list[str] = []

    required_paths = [
        PHASE204_PLAN,
        MIGRATION_PLAN,
        PHASE_STATUS,
        TASKS,
        ASSEMBLY_RUNTIME_SESSION_FACTORY,
        ASSEMBLY_RUNTIME_ASSEMBLY_SESSION_FACTORY,
    ]
    for path in required_paths:
        if not path.exists():
            errors.append(f"missing_required_path:{path}")

    if errors:
        print("[phase204-interaction-session-factory-assembly-runtime-assembly-typed-routing-extraction] FAILED")
        for error in errors:
            print(
                "[phase204-interaction-session-factory-assembly-runtime-assembly-typed-routing-extraction] "
                f"ERROR {error}"
            )
        return 1

    phase204_plan_text = _read(PHASE204_PLAN)
    migration_plan_text = _read(MIGRATION_PLAN)
    phase_status_text = _read(PHASE_STATUS)
    tasks_text = _read(TASKS)
    assembly_runtime_session_factory_text = _read(ASSEMBLY_RUNTIME_SESSION_FACTORY)
    assembly_runtime_assembly_session_factory_text = _read(ASSEMBLY_RUNTIME_ASSEMBLY_SESSION_FACTORY)

    if "## Phase 204 Slice Status" not in phase204_plan_text:
        errors.append("phase204_plan_missing_slice_status")
    if "`204.1` complete." not in phase204_plan_text:
        errors.append("phase204_plan_missing_204_1_complete")
    if "`204.2` complete." not in phase204_plan_text:
        errors.append("phase204_plan_missing_204_2_complete")
    if "`204.3` complete." not in phase204_plan_text:
        errors.append("phase204_plan_missing_204_3_complete")

    if (
        "## Phase 204 (Interaction Session Factory Assembly Runtime Assembly Typed Routing Extraction)"
        not in migration_plan_text
    ):
        errors.append("migration_plan_missing_phase204_section")

    if "PHASE 204 STARTED" not in phase_status_text:
        errors.append("phase_status_missing_phase204_started")
    if "PHASE 204 COMPLETE" not in phase_status_text:
        errors.append("phase_status_missing_phase204_complete")

    required_tasks = [
        "- [x] Define Phase 204 interaction-session factory assembly-runtime assembly typed routing extraction scope and completion evidence gates.",
        "- [x] Consolidate `InteractionSessionFactoryAssemblyRuntimeSessionFactory.createFromAssemblyFactoryInputs(...)` through focused assembly-runtime assembly session routing ownership.",
        "- [x] Run Phase 204 verification + guard pack and mark `PHASE 204 COMPLETE`.",
    ]
    for task_line in required_tasks:
        if task_line not in tasks_text:
            errors.append(f"tasks_missing_phase204_line:{task_line}")

    required_session_factory_strings = [
        "static InteractionSession createFromAssemblyFactoryInputs(",
        "InteractionSessionFactoryAssemblyRuntimeAssemblySessionFactory.createFromAssemblyFactoryInputs(",
        "InteractionSessionFactoryAssemblyFactoryInputsSessionFactory.createFromAssemblyFactoryInputs(",
    ]
    for required_string in required_session_factory_strings:
        if required_string not in assembly_runtime_session_factory_text:
            errors.append(f"assembly_runtime_session_factory_missing_string:{required_string}")

    if "InteractionSessionFactoryAssemblyFactoryInputsSessionFactory.createFromAssemblyFactoryInputs(" not in assembly_runtime_assembly_session_factory_text:
        errors.append("assembly_runtime_assembly_session_factory_missing_assembly_factory_delegate")

    if errors:
        print("[phase204-interaction-session-factory-assembly-runtime-assembly-typed-routing-extraction] FAILED")
        for error in errors:
            print(
                "[phase204-interaction-session-factory-assembly-runtime-assembly-typed-routing-extraction] "
                f"ERROR {error}"
            )
        return 1

    print(
        "[phase204-interaction-session-factory-assembly-runtime-assembly-typed-routing-extraction] OK: "
        "interaction session factory assembly-runtime assembly typed routing extraction Phase 204 baseline is enforced."
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
