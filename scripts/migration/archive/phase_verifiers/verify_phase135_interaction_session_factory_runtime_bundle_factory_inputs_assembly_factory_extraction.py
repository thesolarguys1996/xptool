from __future__ import annotations

from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PHASE135_PLAN = (
    PROJECT_ROOT
    / "docs/NATIVE_CLIENT_PHASE135_INTERACTION_SESSION_FACTORY_RUNTIME_BUNDLE_FACTORY_INPUTS_ASSEMBLY_FACTORY_EXTRACTION_PLAN.md"
)
MIGRATION_PLAN = PROJECT_ROOT / "docs/NATIVE_CLIENT_MIGRATION_PLAN.md"
PHASE_STATUS = PROJECT_ROOT / "docs/NATIVE_CLIENT_PHASE_STATUS.md"
TASKS = PROJECT_ROOT / "TASKS.md"
ASSEMBLY_FACTORY = (
    PROJECT_ROOT
    / "runelite-plugin/src/main/java/com/xptool/sessions/InteractionSessionFactoryRuntimeBundleFactoryInputsAssemblyFactory.java"
)
ASSEMBLY_FACTORY_TEST = (
    PROJECT_ROOT
    / "runelite-plugin/src/test/java/com/xptool/sessions/InteractionSessionFactoryRuntimeBundleFactoryInputsAssemblyFactoryTest.java"
)


def _read(path: Path) -> str:
    return path.read_text(encoding="utf-8")


def main() -> int:
    errors: list[str] = []

    required_paths = [
        PHASE135_PLAN,
        MIGRATION_PLAN,
        PHASE_STATUS,
        TASKS,
        ASSEMBLY_FACTORY,
        ASSEMBLY_FACTORY_TEST,
    ]
    for path in required_paths:
        if not path.exists():
            errors.append(f"missing_required_path:{path}")

    if errors:
        print("[phase135-interaction-session-factory-runtime-bundle-factory-inputs-assembly-factory-extraction] FAILED")
        for error in errors:
            print(
                "[phase135-interaction-session-factory-runtime-bundle-factory-inputs-assembly-factory-extraction] "
                f"ERROR {error}"
            )
        return 1

    phase135_plan_text = _read(PHASE135_PLAN)
    migration_plan_text = _read(MIGRATION_PLAN)
    phase_status_text = _read(PHASE_STATUS)
    tasks_text = _read(TASKS)
    assembly_factory_text = _read(ASSEMBLY_FACTORY)
    assembly_factory_test_text = _read(ASSEMBLY_FACTORY_TEST)

    if "## Phase 135 Slice Status" not in phase135_plan_text:
        errors.append("phase135_plan_missing_slice_status")
    if "`135.1` complete." not in phase135_plan_text:
        errors.append("phase135_plan_missing_135_1_complete")
    if "`135.2` complete." not in phase135_plan_text:
        errors.append("phase135_plan_missing_135_2_complete")
    if "`135.3` complete." not in phase135_plan_text:
        errors.append("phase135_plan_missing_135_3_complete")

    if (
        "## Phase 135 (Interaction Session Factory Runtime Bundle Factory Inputs Assembly Factory Extraction)"
        not in migration_plan_text
    ):
        errors.append("migration_plan_missing_phase135_section")

    if "PHASE 135 STARTED" not in phase_status_text:
        errors.append("phase_status_missing_phase135_started")
    if "PHASE 135 COMPLETE" not in phase_status_text:
        errors.append("phase_status_missing_phase135_complete")

    required_tasks = [
        "- [x] Define Phase 135 interaction-session factory runtime-bundle-factory-inputs assembly-factory extraction scope and completion evidence gates.",
        "- [x] Extract focused `InteractionSessionFactoryRuntimeBundleFactoryInputsAssemblyFactory` ownership for interaction-session assembly-input to runtime-bundle-factory-input mapping seams.",
        "- [x] Run Phase 135 verification + guard pack and mark `PHASE 135 COMPLETE`.",
    ]
    for task_line in required_tasks:
        if task_line not in tasks_text:
            errors.append(f"tasks_missing_phase135_line:{task_line}")

    required_assembly_factory_strings = [
        "final class InteractionSessionFactoryRuntimeBundleFactoryInputsAssemblyFactory",
        "static InteractionSessionFactoryRuntimeBundleFactoryInputs createRuntimeBundleFactoryInputs(",
        "InteractionSessionAssemblyFactoryInputs assemblyFactoryInputs",
        "InteractionSessionFactoryInputs.fromServices(",
        "assemblyFactoryInputs.sessionInteractionKey",
    ]
    for required_string in required_assembly_factory_strings:
        if required_string not in assembly_factory_text:
            errors.append(f"assembly_factory_missing_string:{required_string}")

    required_assembly_factory_test_strings = [
        "class InteractionSessionFactoryRuntimeBundleFactoryInputsAssemblyFactoryTest",
        "createRuntimeBundleFactoryInputsBuildsTypedInputsFromAssemblyInputs",
    ]
    for required_string in required_assembly_factory_test_strings:
        if required_string not in assembly_factory_test_text:
            errors.append(f"assembly_factory_test_missing_string:{required_string}")

    if errors:
        print("[phase135-interaction-session-factory-runtime-bundle-factory-inputs-assembly-factory-extraction] FAILED")
        for error in errors:
            print(
                "[phase135-interaction-session-factory-runtime-bundle-factory-inputs-assembly-factory-extraction] "
                f"ERROR {error}"
            )
        return 1

    print(
        "[phase135-interaction-session-factory-runtime-bundle-factory-inputs-assembly-factory-extraction] OK: "
        "interaction session factory runtime-bundle-factory-inputs assembly-factory extraction Phase 135 baseline is enforced."
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
