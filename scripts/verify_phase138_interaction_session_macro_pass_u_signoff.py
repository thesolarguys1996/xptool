from __future__ import annotations

from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PHASE138_PLAN = PROJECT_ROOT / "docs/NATIVE_CLIENT_PHASE138_INTERACTION_SESSION_MACRO_PASS_U_SIGNOFF_PLAN.md"
MIGRATION_PLAN = PROJECT_ROOT / "docs/NATIVE_CLIENT_MIGRATION_PLAN.md"
PHASE_STATUS = PROJECT_ROOT / "docs/NATIVE_CLIENT_PHASE_STATUS.md"
JAVA_SURFACE_INVENTORY = PROJECT_ROOT / "docs/NATIVE_JAVA_SURFACE_INVENTORY.md"
TASKS = PROJECT_ROOT / "TASKS.md"


def _read(path: Path) -> str:
    return path.read_text(encoding="utf-8")


def main() -> int:
    errors: list[str] = []

    required_paths = [PHASE138_PLAN, MIGRATION_PLAN, PHASE_STATUS, JAVA_SURFACE_INVENTORY, TASKS]
    for path in required_paths:
        if not path.exists():
            errors.append(f"missing_required_path:{path}")

    if errors:
        print("[phase138-interaction-session-macro-pass-u-signoff] FAILED")
        for error in errors:
            print(f"[phase138-interaction-session-macro-pass-u-signoff] ERROR {error}")
        return 1

    phase138_plan_text = _read(PHASE138_PLAN)
    migration_plan_text = _read(MIGRATION_PLAN)
    phase_status_text = _read(PHASE_STATUS)
    java_surface_inventory_text = _read(JAVA_SURFACE_INVENTORY)
    tasks_text = _read(TASKS)

    if "## Phase 138 Slice Status" not in phase138_plan_text:
        errors.append("phase138_plan_missing_slice_status")
    if "`138.1` complete." not in phase138_plan_text:
        errors.append("phase138_plan_missing_138_1_complete")
    if "`138.2` complete." not in phase138_plan_text:
        errors.append("phase138_plan_missing_138_2_complete")
    if "`138.3` complete." not in phase138_plan_text:
        errors.append("phase138_plan_missing_138_3_complete")

    if "## Phase 138 (Interaction Session Macro Pass U Signoff)" not in migration_plan_text:
        errors.append("migration_plan_missing_phase138_section")

    if "PHASE 138 STARTED" not in phase_status_text:
        errors.append("phase_status_missing_phase138_started")
    if "PHASE 138 COMPLETE" not in phase_status_text:
        errors.append("phase_status_missing_phase138_complete")

    required_tasks = [
        "- [x] Define Phase 138 interaction-session macro pass U signoff scope and completion evidence gates.",
        "- [x] Publish updated Java surface inventory and migration-plan/task/status artifacts for Phases 135-138.",
        "- [x] Run Phase 138 verification + guard pack and mark `PHASE 138 COMPLETE`.",
    ]
    for task_line in required_tasks:
        if task_line not in tasks_text:
            errors.append(f"tasks_missing_phase138_line:{task_line}")

    required_migration_sections = [
        "## Phase 135 (Interaction Session Factory Runtime Bundle Factory Inputs Assembly Factory Extraction)",
        "## Phase 136 (Interaction Session Factory Runtime Bundle Assembly Entry Typed Routing Extraction)",
        "## Phase 137 (Interaction Session Factory Wiring Consolidation U)",
    ]
    for migration_section in required_migration_sections:
        if migration_section not in migration_plan_text:
            errors.append(f"migration_plan_missing_required_section:{migration_section}")

    required_inventory_lines = [
        "- Phase 135 extracted focused interaction-session runtime-bundle-factory-input assembly mapping ownership",
        "- Phase 136 extracted typed runtime-bundle-factory assembly-entry routing through `InteractionSessionFactoryRuntimeBundleFactoryInputsAssemblyFactory` ownership",
        "- Phase 137 consolidated `InteractionSessionFactory.createFromAssemblyFactoryInputs(...)` seam through typed runtime-bundle-factory-input assembly mapping ownership",
    ]
    for inventory_line in required_inventory_lines:
        if inventory_line not in java_surface_inventory_text:
            errors.append(f"java_surface_inventory_missing_line:{inventory_line}")

    if errors:
        print("[phase138-interaction-session-macro-pass-u-signoff] FAILED")
        for error in errors:
            print(f"[phase138-interaction-session-macro-pass-u-signoff] ERROR {error}")
        return 1

    print("[phase138-interaction-session-macro-pass-u-signoff] OK: interaction session macro pass U signoff Phase 138 baseline is enforced.")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
